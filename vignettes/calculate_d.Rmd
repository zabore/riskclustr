...
Then we can supply this to the `formula` argument of `d()`, along with specification of the name of the subtype variable, the number of subtypes, and the dataset. Note that the variable name supplied to the `label` argument must be a quoted string. The subtype variable must be a numeric variable where controls are coded as 0 and cases are coded as 1 through M according to the subtype to which they belong. 
...
Then we specify the arguments to `dstar()` as before, but supplying our new case-only dataset to the `data` argument. Now, the subtype variable must be numeric, with cases coded as 1 through M according to the subtype to which they belong. 
