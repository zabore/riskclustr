...
This tutorial will introduce you to the `optimal_kmeans_d()` function, which will run $k$-means clustering on disease marker data and identify the subtype solution that maximizes $D$. This function is currently for use with case-control data only. See [Tutorial: Estimate the extent of etiologic heterogeneity](calculate_d.html) for details on the calculation of the $D$ value.
...
Let's say we wanted to identify the optimally etiologically heterogeneous 3-subtype solution based on all 30 markers `y1`-`y30` available in `subtype_data`, using all three risk factors `x1`, `x2` and `x3`. We use the default of 100 random starts of the $k$-means clustering algorithm, and set a seed so that results would be reproduced if we were to run this code again on a later date. Note that this function is a bit slow to run, so please be patient.
...
First let's look at a tabulation of `optimal_d_label` from the `optimal_d_data` object:
