...
An overall formatted dataframe containing $\hat{\mathbf{\beta}}$  ($\sqrt{\widehat{var}(\hat{\mathbf{\beta}})}$) and heterogeneity p-values `p_het` to test the null hypotheses $H_{0_{\beta_{p.}}}$ can be obtained as
...
The estimates and heterogeneity p-values for disease subtypes formed by cross-classifying these individual tumor markers can also be accessed in objects `beta_se_p` and `or_ci_p`, as described in the section on [Pre-specified subtypes](#subtype).
