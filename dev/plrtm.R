################################################################################
#' Conduct an analysis of etiologic heterogeneity for individual tumor markers
#' using polytomous logistic regression
#'
#' @author Emily C Zabor \email{zabore@@mskcc.org}
#'
#' @description \code{plrtm} takes a list of individual tumor markers and a
...
plrtm <- function(tm, rf, case, data) {

  library(nnet)
  library(aod)

  # Check if levels of each element in tm have only values 0 or 1
  if(any(sapply(lapply(tm, function(x) {levels(as.factor(data[, x]))}),
  ...
  data <- merge(data, st, by = tm, all = T)
  ...
  rownames(beta_plr) <- rownames(beta_se) <- levels(data$sub_name)
  ...
  colnames(or_ci_p) <- colnames(beta_se_p) <- c(levels(data$sub_name), "p_het")
